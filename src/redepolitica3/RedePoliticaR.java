...
public class RedePoliticaR
{
    ...
    public class Relacao
    {
        ...
        LargeInteger numeradorPercentualRepasse;
        public Relacao(Candidato relecionado, LargeInteger numeradorPercentualRepasse)
        {
            this.relacionado = relecionado;
            this.numeradorPercentualRepasse = numeradorPercentualRepasse;            
        }        
    ...
    public class Candidato
    {
        ...
        boolean isBandeira;         
        LargeInteger denominadorComumPercentualRepasse;
        boolean updatedRationals;
        ...
        void preparaParaInicioDaApuracao()
        {
            ...
            if (!vizinhosCorrentes.isEmpty())
            {                    
                LargeInteger lcm = null;
                for (Relacao r:vizinhosCorrentes.values())
                {    
                    LargeInteger denominator = r.percentualRepasse.getDivisor();
                    if (lcm == null)                    
                        lcm = denominator;
                    else
                        lcm = lcm.times(denominator).divide(lcm.gcd(denominator));
                }                                        
                denominadorComumPercentualRepasse = lcm;
                for (Relacao r:vizinhosCorrentes.values())
                {    
                    LargeInteger denominator = r.percentualRepasse.getDivisor();
                    LargeInteger numerator = r.percentualRepasse.getDividend();
                    LargeInteger multiplier = lcm.divide(denominator);
                    r.numeradorPercentualRepasse = multiplier.times(numerator);
                    //r.percentualRepasse = null;
                }                                                        
                updatedRationals = false;
            }                            
        }        
        ...
        private void updateRationals()
        {
            for (Relacao r:vizinhosCorrentes.values())
               r.percentualRepasse = Rational.valueOf(r.numeradorPercentualRepasse, denominadorComumPercentualRepasse);            
        }
    }
    public String toString()
    {
    ...
    private void removeDoConjuntoDeVizinhosSemAjustarContravizinhos(Candidato candidatoASerRemovido,Candidato candidatoDeOndeRemover)
    {
        ...
        {
            //Rational percentualIdaS = candidatoDeOndeRemover.vizinhosCorrentes.get(candidatoASerRemovido.numero).percentualRepasse;
            //Rational percentualVoltaS = Rational.ZERO;            
            //if (candidatoASerRemovido.vizinhosCorrentes.containsKey(candidatoDeOndeRemover.numero))
            //       percentualVoltaS = candidatoASerRemovido.vizinhosCorrentes.get(candidatoDeOndeRemover.numero).percentualRepasse;
            
            {
                LargeInteger idaMult = candidatoDeOndeRemover.vizinhosCorrentes.get(candidatoASerRemovido.numero).numeradorPercentualRepasse;
                ...
                Rational percentualIda =   Rational.valueOf
                                           (
                                                idaMult,
                                                candidatoDeOndeRemover.denominadorComumPercentualRepasse                                              
                                           );

                Rational percentualVolta = Rational.ZERO;            
                if (candidatoASerRemovido.vizinhosCorrentes.containsKey(candidatoDeOndeRemover.numero))
                       percentualVolta =   Rational.valueOf
                                           (
                                               candidatoASerRemovido.vizinhosCorrentes.get(candidatoDeOndeRemover.numero).numeradorPercentualRepasse,
                                               candidatoASerRemovido.denominadorComumPercentualRepasse
                                           );
                ...
                Rational gCoef = Rational.valueOf(
                                                    percentualRestandeDeIdaEVolta.getDivisor(), 
                                                    percentualRestandeDeIdaEVolta.getDividend().times(candidatoDeOndeRemover.denominadorComumPercentualRepasse).times(candidatoASerRemovido.denominadorComumPercentualRepasse)
                                                 );

                LargeInteger gMult = gCoef.getDividend();
                LargeInteger gDiv = gCoef.getDivisor();

                LargeInteger aMult = gMult.times(candidatoASerRemovido.denominadorComumPercentualRepasse);
                LargeInteger bMult = gMult.times(idaMult);

                candidatoDeOndeRemover.denominadorComumPercentualRepasse = gDiv;

                System.out.println(gDiv);
                
                for (Relacao r:candidatoDeOndeRemover.vizinhosCorrentes.values())
                    r.numeradorPercentualRepasse = r.numeradorPercentualRepasse.times(aMult);
                ...
                for (Relacao r:candidatoASerRemovido.vizinhosCorrentes.values())
                {
                    ...
                    if (r2==null)
                    {    
                        r2 = new Relacao(r.relacionado,LargeInteger.ZERO);
                        candidatoDeOndeRemover.vizinhosCorrentes.put(r2.relacionado.numero, r2);   
                    }    
                    r2.numeradorPercentualRepasse = r2.numeradorPercentualRepasse.plus(r.numeradorPercentualRepasse.times(bMult));       
                }                      

                candidatoDeOndeRemover.updatedRationals = false;
            }
/*            
            {
                Rational percentualIda = percentualIdaS;
                Rational percentualVolta = percentualVoltaS;
                ...
                for (Relacao r:candidatoASerRemovido.vizinhosCorrentes.values())
                {
                    ...
                    Rational pr = Rational.valueOf(r2.numeradorPercentualRepasse, candidatoDeOndeRemover.denominadorComumPercentualRepasse);
                    
                    if (!pr.equals(r2.percentualRepasse))
                       throw new RuntimeException("Erro na denominação comum");
                }            
            }            
*/            
        }            
